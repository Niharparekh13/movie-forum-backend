...
import { RegisterDto, LoginDto } from '../validators/auth.dto.js';
import { error, ok } from '../utils/response.js';
export async function register(req, res) {
    const parse = RegisterDto.safeParse(req.body);
    if (!parse.success)
        return res.status(422).json(error('VALIDATION_ERROR', 'Invalid input', parse.error.issues));
    const { username, email, password } = parse.data;
    const exists = await prisma.user.findFirst({ where: { OR: [{ email }, { username }] } });
    if (exists)
        return res.status(409).json(error('CONFLICT', 'Email or username already exists'));
